...
class CarlosSchedulingHandler:
    ...
    async def handle(self, message: InboundMessage) -> None:
        ...
        if existing is not None:
            logger.info(
                "Carlos scheduling reply already exists: inbound_record_id=%s instance=%s",
                message.id,
                message.instance,
            )
            return
        ...
        logger.info(
            "Carlos scheduling reply enqueued: inbound_record_id=%s outbound_record_id=%s "
