...
class CarlosSchedulingWriteService:
    def __init__(
        self,
        *,
        session_factory: async_sessionmaker[AsyncSession],
        openai_client: OpenAIResponsesClient,
        tool_executor: SchedulingWriteToolExecutor,
        settings: Settings,
    ...
    async def generate_reply(self, message: InboundMessage) -> str:
        ...
        context = await prepare_carlos_context(
            self._session_factory, message, self._settings, now_utc=self._clock.now_utc()
        )
        ...
        availability_decision = validate_availability_request(
            context.state, today=local_today,
        ...
        for round_index in range(self._settings.openai_max_tool_rounds + 1):
            ...
            logger.info(
                "Executing Carlos scheduling write tool: inbound_record_id=%s tool_name=%s round=%s",
                message.id,
                tool_call.name,
                round_index + 1,
            )
    ...
    def _build_instructions(self) -> str:
        ...
        return f"{CARLOS_SCHEDULING_WRITE_SYSTEM_PROMPT}\n\n{context}"
